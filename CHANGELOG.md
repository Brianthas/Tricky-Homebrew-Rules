...
- **Spirit Guardians deliberately does not use it.** dnd5e's 2024 spell ships its own Half Speed effect, whose changes multiply walk, fly, climb, swim and burrow by 0.5, and radiating that effect is what the aura was already for: a Ghoul in range measured 30 walking speed before and 15 after. A terrain region on top would halve the speed and double the cost, leaving a quarter of normal movement. Checked the rest of the known-auras table the same way - Aura of Life, Beacon of Hope and Antilife Shell all ship their own effects too, and none of them asks for terrain. Conjure Minor Elementals ships no effect at all, which is why it needs one built by hand and why it is the entry that wants the region.
